...
int CRemoteClientDlg::SendCommandPacket(int nCmd, bool bAutoClose = true, BYTE* pData = NULL, size_t nLength = 0)
{
	UpdateData();
	CClientSocket* pClinet = CClientSocket::getInstance();
	bool ret = pClinet->InitSocket(m_server_address, atoi((LPCTSTR)m_nPort));
	if (!ret) {
		AfxMessageBox("网络初始化失败！");
		return -1;
	}
	CPacket pack(nCmd, pData, nLength);
	ret = pClinet->Send(pack);
	TRACE("Send ret %d\r\n", ret);
	int cmd = pClinet->DealCommand();
	TRACE("ack:%d\r\n", cmd);
	if (bAutoClose) 
		pClinet->CloseSocket();
	return cmd;
...
void CRemoteClientDlg::OnBnClickedBtnFileinfo()
{
	...
	CClientSocket* pClinet = CClientSocket::getInstance();
	std::string drivers = pClinet->GetPacket().strData;
	std::string dr;
	...
	for (size_t i = 0; i < drivers.size(); i++) {
		if (drivers[i] == ',') {
			...
			m_Tree.InsertItem(dr.c_str(), hTemp, TVI_LAST);
			dr.clear();
			continue;
		}
		dr += drivers[i];
